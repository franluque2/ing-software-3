...
#### 1- Conceptos de Dockerfiles
  ...
  - Describir las instrucciones
     - FROM: Nos dice que imagen vamos a usar como base del contenedor.
     - RUN: ejecutar un comando en un shell.
     - ADD: Copiar archivos desde una url o dirección.
     - COPY: copiar archivos de una fuente a un destino.
     - EXPOSE: Escuchar en un puerto.
     - CMD: Ejecutar un comando dentro del contenedor.
     - ENTRYPOINT: Configurar que pasa cuando el contenedor se ejecuta como un ejecutable (?)
...
Traemos la imagen del java development kit 8 de maven, copiando el nuevo pom en temp. Ahi corremos mvn para las dependencias, utilizando el pom copiado segun la configuracion de settings.

Luego copiamos el source, y establecemos el directorio principal en tmp.

Corremos y empaquetamos el proyecto.

Posteriormente, tomamos el run enviroment de java 8, escuchando en el puerto 8080, ahí creamos una nueva carpeta y copiamos del jdk el jar a la aplicación.

Seteamos ciertas opciones de memoria de java y establecemos su entry point.

Finalmente con healthcheck revisamos el funcionamiento del container. 
...
```
  ...
  Crea el contenedor, y lo manda al fondo al estar detatched, por lo que no ocupa la consola.
